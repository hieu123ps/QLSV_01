...
def loadData():
    ...
    mycursor.execute("select a.accs_id, a.accs_prsn, b.prs_name, b.prs_skill, date_format(a.accs_added,'%d/%m/2023 (%H:%i)') "
                     "  from accs_hist a "
                     "  left join prs_mstr b on a.accs_prsn = b.prs_nbr "
                     " where a.accs_date = curdate() "
                     " order by 1 desc")
...
#     mycursor = mydb.cursor()
#     try:
#         df=sql.read_sql('select * from accs_hist',mydb)
#         print(df)
#         df.to_excel('ds.xls')
...
#     if target not in data[0][0]:
#         return render_template("error.html", target)
#     else:
#         return data[0][0]   

# def binary_search(theSeq, target):
#     left, right = 0, len(theSeq)
#     mid = (left+right)//2
#     if theSeq[mid][0] == target:
#         return True
#     else:
#         if(theSeq[mid][0] > target):
#             return binary_search(theSeq[mid][0], target)
